...
La causa está en el invariante del algoritmo. Cada elemento se coloca sobre la primera pila cuyo tope sea mayor o igual a él, y crea una pila nueva si no existe tal pila. Sobre una entrada estrictamente creciente, cada elemento es mayor que todos los topes y genera una pila nueva, de modo que el número de pilas crece hasta $N$; sobre una entrada decreciente, todo elemento es menor o igual al tope de la primera pila y el algoritmo termina con una sola pila. El número de pilas es, en consecuencia, el largo de la subsecuencia decreciente mínima en que se descompone la entrada, y es este valor, no $N$, el que determina el costo real: la memoria auxiliar es $\Theta(\text{pilas})$ y la fusión final mantiene una cola de prioridad de ese mismo tamaño.
...
\begin{itemize}
    \item \textbf{Coincidencia forzada para $N \le 64$.} La implementación de Strassen conmuta al algoritmo iterativo cuando la dimensión del bloque alcanza 64. En consecuencia, para $N = 16$ y $N = 64$ ambos programas ejecutan exactamente el mismo código y las diferencias medidas (a lo sumo 0{,}05 ms, con signo que cambia arbitrariamente entre configuraciones) son ruido de medición y no una comparación entre algoritmos. Este umbral no es arbitrario: por debajo de él, el costo de asignar y copiar las submatrices de cada nivel de recursión excede el ahorro de pasar de ocho a siete multiplicaciones.

    \item \textbf{Cruce asintótico.} En $N = 256$ el resultado es indistinguible: Strassen es más rápido en 3 de las 6 configuraciones y más lento en las otras 3. En $N = 1024$, en cambio, Strassen es más rápido en las 6 configuraciones, con razones de 1{,}07 a 2{,}01. El cruce práctico se sitúa así entre $N = 256$ y $N = 1024$, muy por encima del punto en que la ventaja teórica ($N^{2{,}81}$ frente a $N^{3}$) comienza a existir.
